...
<?php
	...
	{
		$list = mysqli_fetch_assoc(mysqli_query("SELECT count(*) as total,id FROM feedback_reviews WHERE reviewer_id='".$_GET['loginid']."' && patient_id='".$_GET['patientid']."' && groups_id='".$_GET['group_id']."'"));
		if(isset($_GET['ticket']))
			mysqli_query("update feedback_reviews set ticket_id='1' WHERE id='".$list['id']."'");
		else if($list['total']<1)
			mysqli_query("INSERT INTO feedback_reviews (groups_id,patient_id,reviewer_id,review,ticket_id,date_time) VALUES ('".$_GET['group_id']."','".$_GET['patientid']."','".$_GET['loginid']."','1','','".date('Y-m-d H:i:s')."')");
	}
?>
</table>