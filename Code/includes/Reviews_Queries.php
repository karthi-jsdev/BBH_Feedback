<?php
	...
	{
		return mysql_query("SELECT feedbacks_".$_GET['group_id'].".id,patients.name as pname,patients.patient_id,survey_id,groups.name,patients.email,
	...
	{
		/* return mysql_query("SELECT Count(feedbacks_".$_GET['group_id'].".id)As total FROM feedback_reviews 
								join groups on feedback_reviews.groups_id = groups.id
								join patients on groups.id=patients.group_id 
								join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
								WHERE patients.group_id='".$_GET['group_id']."'"); */
		return mysql_query("SELECT Count(feedbacks_".$_GET['group_id'].".id)As total FROM groups
								join patients on groups.id=patients.group_id 
								join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
							");
	}
	...
	{
		return mysql_query("SELECT patients.name,patients.patient_id,groups.prefix,patients.id as pid,feedbacks_".$_GET['group_id'].".survey_id,feedbacks_".$_GET['group_id'].".id as id
							FROM groups
							join patients on groups.id=patients.group_id 
							join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
							LIMIT $Start,$Limit");
	...
	function QAList()
	{
		return mysql_query("SELECT column_name FROM information_schema.columns WHERE table_name = feedbacks_".$_GET['group_id']."");
	}
	function Patient_FeedbackNotReviewedCount()
	{
		return mysql_query("SELECT COUNT(*) as total
							FROM  groups
							join patients on groups.id=patients.group_id 
							join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
	...
	{
		return mysql_query("SELECT feedbacks_".$_GET['group_id'].".id as id,patients.id as pid,patients.name,patients.patient_id,groups.prefix,feedbacks_".$_GET['group_id'].".survey_id
							FROM  groups
							join patients on groups.id=patients.group_id 
							join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
	...
	{
		return mysql_query("SELECT feedbacks_".$_GET['group_id'].".id,patients.name,patients.id as pid,patients.patient_id,groups.prefix,feedbacks_".$_GET['group_id'].".survey_id
							FROM  groups
							join patients on groups.id=patients.group_id 
							join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
	...
	{
		return mysql_query("SELECT feedbacks_".$_GET['group_id'].".id,patients.name,patients.patient_id,review_msg,ticket_id,review,ticket_msg,ticket_no,feedbacks_".$_GET['group_id'].".survey_id
							FROM  groups
							join patients on groups.id=patients.group_id 
							join feedbacks_".$_GET['group_id']." on feedbacks_".$_GET['group_id'].".patient_id=patients.id
