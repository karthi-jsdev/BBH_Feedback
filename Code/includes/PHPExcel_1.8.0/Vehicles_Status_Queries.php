<?php
	...
	{
		return mysql_query("SELECT COUNT(*) AS total FROM `log` WHERE deviceId=".$_POST['deviceId']." && (log.logType=".str_replace(",", " || log.logType=", $_POST['type']).") && dateTime>='".date("Y-m-d H:i:s",strtotime($_POST['startdate']))."' && dateTime<='".date("Y-m-d H:i:s",strtotime($_POST['enddate']))."' ORDER BY id");
	}
	function Select_Device_Reports($Start, $Limit)
	{
		...
		return mysql_query("SELECT log.*, devices.deviceName, devices.tcpStatus FROM `log`
