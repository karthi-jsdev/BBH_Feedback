...
$(document).ready(function() {
	...
	$('#myMenu1 .answerOptions').click(function (){ treeOps.answerOptions(); });
	...
	//$('#myMenu1 .answerOptions').append(langManager.treeOps.answerOptions);
		...
		afterContextMenu: function(element, event)
		{
			...
			if (className.indexOf('doc') >= 0) {
				...
				if (className.indexOf('root') > 0) {
					...
					$('#myMenu1 .addFolder').show();
					$('#myMenu1 .expandAll, #myMenu1 .collapseAll').show();
				}
				else if(className.indexOf('root') == 0)
				{
					$('#myMenu1 .answerOptions, #myMenu1 .addDoc').hide();
					$('#myMenu1 .expandAll, #myMenu1 .collapseAll').show();
					$('#myMenu1 .addFolder').show();
					$('#myMenu1 .edit, #myMenu1 .delete').hide();
				}	
				else {
					$('#myMenu1 .addFolder').hide();
					$('#myMenu1 .expandAll, #myMenu1 .collapseAll').hide();
				}
				$('#myMenu1').css('top',event.pageY).css('left',event.pageX).show();
			}
			
			$('*').click(function() { $('#myMenu1, #myMenu2').hide(); $('#myMenu1 .edit,#myMenu1 .answerOptions,#myMenu1 .addDoc, #myMenu1 .delete').show(); });
