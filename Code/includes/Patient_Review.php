		...
		<form action="" method="POST" class="form panel">
			...
			<?php
				if(isset($_GET['patient_id']))
				{
					...
					$status = mysqli_Fetch_Assoc($Patient_Status);
				/* 	if($status['ticket_id']!=1)
					...
					while($Feedback = mysqli_fetch_array($FeedbacksQuery))
					{
						...
						for($i = 0; $i < COUNT($Questions['id']); $i++)
						{
							...
							$Comment_total = mysqli_fetch_array(mysqli_query("SELECT count(*) as total FROM feedbacks_comments WHERE question_id=".$Questions['id'][$i]." ORDER BY id DESC LIMIT 1"));
					...
					{
						$list = mysqli_fetch_assoc(mysqli_query("SELECT count(*) as total,id FROM feedback_reviews WHERE reviewer_id='".$_GET['loginid']."' && patient_id='".$_GET['patient_id']."' && groups_id='".$_GET['group_id']."' && feedbackid='".$_GET['feedbackid']."'"));
						if($list['total']<1)
						{
							if($_POST['review_msg']=="")
								echo "<br /><div class='message error'><b>Message</b> : Please specify Some Review Information</div>";
							else
								mysqli_query("INSERT INTO feedback_reviews (groups_id,patient_id,feedbackid,reviewer_id,review_msg,review,ticket_id,ticket_msg,date_time) VALUES ('".$_GET['group_id']."','".$_GET['patient_id']."','".$_GET['feedbackid']."','".$_GET['loginid']."','".$_POST['review_msg']."','1','','','".date('Y-m-d H:i:s')."')");
						}
						else
						{
							...
							if($_POST['ticketno'])
							{
								...
								$Ticket = mysqli_fetch_assoc(mysqli_query("Select * from complaint where ticketno like '%".$data[1]."%' ORDER BY ID DESC LIMIT 0,1"));
								$TicketNo = explode("-",$Ticket['ticketno']);
								$TicketNo = $data[1]."-".$Digits[7 - strlen($TicketNo[1]+1)].($TicketNo[1]+1);
								$date = date('Y-m-d h-i-s');
								$execute = mysqli_query("INSERT INTO complaint (ticketno,description,remarks,groupid,subgroupid,sourceid,createdat,updatedat,statusid,priorityid,updatedby,assignedto,createdby,departmentid) VALUES ('".$TicketNo."','".$_POST['ticket_msg']."','".$patientinfo['survey_id']."','".$data[0]."','13','6','".$date."','".$date."','1','3','247','247','247','121')");
								$ComplaintId = mysqli_fetch_assoc(mysqli_query("SELECT id FROM complaint WHERE ticketno='".$TicketNo."'"));
								$audit = mysqli_query("INSERT INTO audit(complaintid, comments, statusid, priorityid, addedby, addedat) VALUES('".$ComplaintId['id']."','New ticket created by Survey admin','1','3','247','".$date."')");
								//mysqli_query("INSERT INTO complaint (ticketno,description,remarks,groupid,subgroupid,sourceid,createdat,updatedat,statusid,priorityid) VALUES ('".$TicketNo."','".$_POST['ticket_msg']."','".$patientinfo['survey_id']."','".$data[0]."','13','6','".$date."','".$date."','1','3')");
								if($audit==1 && $execute==1)
								{
									include("includes/Config.php");
									mysqli_query("update feedback_reviews set ticket_id='1',ticket_msg='".$_POST['ticket_msg']."',ticket_no='".$TicketNo."' WHERE id='".$list['id']."'");
								}
							}
						}
						?>
					<table class="paginate sortable full">
						...
						if(($execute!=1 || $audit!=1) && $_POST['ticket'])
						{
							...
							include("includes/Config_bbhm.php");
							$groups = mysqli_query("SELECT * FROM `group` ORDER BY id DESC");
